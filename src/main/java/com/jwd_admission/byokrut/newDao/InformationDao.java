...
public class InformationDao extends BaseDao<Integer, PersonalInformation> {
    ...
    public PersonalInformation findEntityById(Integer id) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(SELECT_INF_BY_ID)) {
    ...
    public List<PersonalInformation> findAll() {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(SELECT_ALL_INF)) {
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
                String name = rs.getString(nameField);
                String middlename = rs.getString(middlenameField);
    ...
    public boolean delete(Integer id) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(DELETE_USER_INF_BY_ID)) {
            preparedStatement.setInt(1, id);
    ...
    public boolean create(PersonalInformation personalInformation) {
        try (PreparedStatement preparedStatement1 = connection.prepareStatement(CREATE_USER_INF)) {
    ...
    public PersonalInformation update(PersonalInformation personalInformation) {
        try (PreparedStatement preparedStatement1 = connection.prepareStatement(UPDATE_USER_INF_BY_ID)) {
            preparedStatement1.setString(1, personalInformation.getFirstName());
    ...
    public int findIdByPassportId(String passportId) {
        try (PreparedStatement preparedStatement = connection.prepareStatement(SELECT_INFORMATION_ID_BY_PASSPORT_ID)) {
